...
class VerificationType(str, Enum, metaclass=utils.OpenEnumMeta):
...
class PlaidStatus(str, Enum, metaclass=utils.OpenEnumMeta):
    CONNECTED = "connected"
...
class CompanyBankAccount(BaseModel):
    ...
    def serialize_verification_status(self, value):
        if isinstance(value, str):
            try:
                return models.VerificationStatus(value)
            except ValueError:
                return value
        return value

    @field_serializer("verification_type")
    def serialize_verification_type(self, value):
        if isinstance(value, str):
            try:
                return models.VerificationType(value)
            except ValueError:
                return value
        return value

    @field_serializer("plaid_status")
    def serialize_plaid_status(self, value):
        if isinstance(value, str):
            try:
                return models.PlaidStatus(value)
            except ValueError:
                return value
        return value
